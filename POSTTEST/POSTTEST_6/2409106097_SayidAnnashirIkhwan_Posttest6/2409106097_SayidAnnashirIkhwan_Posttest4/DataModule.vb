...
Module DataModule
    ...
    Public Function InsertPokemon(namaPengguna As String, namaPokemon As String) As Boolean
        Try
            Dim query As String = "INSERT INTO pokemon_terpilih (nama_pengguna, nama_pokemon) VALUES (@nama, @pokemon)"

            Using conn = GetConnection2()
                conn.Open()
                Using cmd As New MySqlCommand(query, conn)
                    cmd.Parameters.AddWithValue("@nama", namaPengguna)
                    cmd.Parameters.AddWithValue("@pokemon", namaPokemon)
                    cmd.ExecuteNonQuery()
                End Using
            End Using
            ...
            Return True
        Catch ex As Exception
            MessageBox.Show("Gagal simpan pokemon: " & ex.Message)
            Return False
        End Try
    End Function
    ...
    Public Function SearchData(keyword As String) As DataTable
        Dim dt As New DataTable()
        Dim query As String = "SELECT * FROM tbtrainer WHERE id LIKE @key OR nama LIKE @key OR region LIKE @key OR kontak LIKE @key OR hobi LIKE @key OR peran LIKE @key"
